import React from 'react';
import imgLogo from "../../images/logo-nav.png";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faEnvelope, faFaceAngry, faTextWidth } from '@fortawesome/free-solid-svg-icons'


const Footer = () => {
    return (
        <div className='py-5  bg-black text-white'>
            <section className='flex flex-row items-center justify-between mx-20'>
                <div>
                    <img className='' src={imgLogo} alt="" />
                </div>
                <div>
                    <ul className="menu menu-horizontal px-1">
                        <li><a>Home</a></li>
                        <li><a>Our Services</a></li>
                        <li><a>About us</a></li>
                        <li><a>Contact us</a></li>
                    </ul>
                </div>
                <div className='flex flex-row items-center justify-center gap-3'>

                    <FontAwesomeIcon icon={faEnvelope} />
                    <FontAwesomeIcon icon={faFaceAngry} />
                    <FontAwesomeIcon icon={faTextWidth} />

                    <svg viewBox="0 0 20 20" aria-hidden="true" class="h-5 w-5 fill-slate-400">
                        <path d="M6.29 18.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0 0 20 3.92a8.19 8.19 0 0 1-2.357.646 4.118 4.118 0 0 0 1.804-2.27 8.224 8.224 0 0 1-2.605.996 4.107 4.107 0 0 0-6.993 3.743 11.65 11.65 0 0 1-8.457-4.287 4.106 4.106 0 0 0 1.27 5.477A4.073 4.073 0 0 1 .8 7.713v.052a4.105 4.105 0 0 0 3.292 4.022 4.095 4.095 0 0 1-1.853.07 4.108 4.108 0 0 0 3.834 2.85A8.233 8.233 0 0 1 0 16.407a11.615 11.615 0 0 0 6.29 1.84"></path>
                    </svg>
                    
                </div>
            </section>

            <div className='mx-20 my-5'>
                <hr />
            </div>
            <div className='flex flex-row items-center justify-between mx-20 mt-3 text-xs'>
                <p>Terms & Condition</p>
                <p>Privacy Policy</p>
            </div>
        </div>
    );
};

export default Footer;